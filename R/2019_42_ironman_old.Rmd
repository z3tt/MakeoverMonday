...
cols <- c("#ebc600", "#cacaca", "#d59352") #"#d4d4d4", "#cd7f32")    ## gold, silver, bronze
```
...
bars_male <- 
  df_iron_sum %>% 
  filter(Gender == "Male") %>% 
  ggplot(aes(id, n, color = Place, fill = Place)) +
...
labs <- 
  ...
  ggplot(aes(1, rank)) +
    ...
    labs(x = NULL, y = NULL,
         ...
         title = "This year, history was made at the Ironman World Championships with German athletes winning both the men's and women's races for the first time.\nGermany has dominated the men's race in recent years, with the last six winners coming from the country and became the toughest adversary of the USA in terms of total medals won.",
...
time_male <- 
  ...
  ggplot(aes(Year, pos, color = Place)) +
    ...
    theme(plot.subtitle = element_text(hjust = 0),
          axis.text.y = element_text(family = "Titillium Web", color = "grey25"),
          axis.ticks = element_blank(),
          panel.grid.major.y = element_blank()) +
    labs(x = NULL, y = NULL,
         subtitle = "IRONMAN️")
...
time_female <- 
  ...
  ggplot(aes(Year, pos, color = Place)) +
    ...
    scale_y_continuous(limits = c(0.6, 16.4), 
                       ...
                       position = "right") +
    scale_color_manual(values = cols, guide = F) +
    theme(plot.subtitle = element_markdown(hjust = 1),
          axis.text.y = element_text(family = "Titillium Web", color = "grey25"),
          axis.ticks = element_blank(),
          panel.grid.major.y = element_blank()) +
    labs(x = NULL, y = NULL,
         subtitle = "IRON<span style='color:#ebc600'>WO</span>MAN")
```

```{r full-panel, fig.width = 14, fig.height = 6.3}
time_male + pyramid + time_female + plot_layout(nrow = 1, widths = c(0.6, 1, 0.6))

ggsave(here::here("plots", "2019_42_ironman_old.png"), 
       width = 14, height = 6.3, dpi = 1200)
```
